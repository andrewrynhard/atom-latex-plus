...
export default {
  ...
  async activate() {
    ...
    this.commands = atom.commands.add("atom-workspace", {
      ...
      "latex-plus:compile": atom.config.transact(async () => {
        this.saveAll();
        try {
      ...
      "latex-plus:compile-and-sync": atom.config.transact(async () => {
        this.saveAll();
        try {
          await this.setProject();
        }
      ...
      "latex-plus:sync": atom.config.transact(async () => {
        if (! this.project) {
          try {
            await this.setProject();
          }
      ...
      "latex-plus:edit": atom.config.transact(async () => {
        if (! this.project) {
          try {
            await this.setProject();
          }
      ...
      "latex-plus:clean": atom.config.transact(async () => {
        if (! this.project) {
          try {
            await this.setProject();
          }
  ...
  async openOutput() {
    ...
    await deleteFile(syncTexPath)
      .then(function(res) { if (res) { symlink(syncTexCompilationPath, syncTexPath); }})
    ...
    if (this.project.postAction == "link") {
      await deleteFile(itemPath)
        .then(function(res) { if (res) { symlink(itemCompilationPath, itemPath); }})
        .catch(errorCallback.bind(this, "symlinking", itemCompilationPath, itemPath));
    } else {
      await deleteFile(itemPath)
        .then(function(res) { if (res) { copy(itemCompilationPath, itemPath); }})
