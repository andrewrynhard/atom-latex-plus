...
export default {
  ...
  activate() {
    ...
    this.subscriptions.add(this.compiler.onDidCompileSuccess(() => {
      ...
      // copy and open the pdf upon successful compilation
      pdf = path.basename(this.project.PDF)
      destination = path.join(this.project.path, pdf);
      fs.exists(destination, (exist) => {
        if (exists) {
          fs.unlink(destination, () => {
            fs.createReadStream(this.project.PDF)
      ...
      for (pane of atom.workspace.getPaneItems()) {
        if (pane.filePath === destination) {
          return;
        }
      }

      pane = atom.workspace.getActivePane();
      pdfPane = pane.splitRight();
      atom.workspace.open(destination, pdfPane);
